...
- Account KPI scoring uses the POC scoring model discipline plus the current scoring framework: every KPI is scored on a 1-5 scale, where 3/5 is the neutral default for missing or unproven account activity evidence.
- If no source-backed evidence exists for a KPI or sub-parameter, assign 3/5, set confidence below 0.85, mark it as needing confirmation, and state clearly that the score is neutral because evidence is missing.
- Never infer high or low scores from company name, ARR tier, industry, source filename, or generic account assumptions. Scores must come from extracted text, user-approved data, current draft data, or explicit user confirmation.
- The standard KPI dimensions are Relationship Score (20%), Contract Health Score (15%), CSAT Score (15%), Risk Score (15%), Resource Health Score (10%), Project Health Score (10%), Financial Score (10%), and Whitespace Analysis (5%).
- KPI reasoning must follow the POC formulas: CSAT Score = 50% CSAT/client feedback KPI + 50% stakeholder touchpoint sentiment; Relationship Score = 25% stakeholder depth + 25% stakeholder breadth + 25% meeting cadence + 25% executive access; Risk Score = 25% delivery risk + 25% commercial risk + 25% relationship risk + 25% market risk; Contract Health Score = 33% renewal risk + 33% contractual protection + 33% commercial foundation; Project Health Score = 33% delivery execution quality + 33% backlog health + 33% velocity health; Resource Health Score = 25% team stability + 25% team fit + 25% turnover risk + 25% bench risk; Financial Score = 33% payment timeliness + 33% outstanding invoices + 33% revenue trend; Whitespace Analysis = 25% expansion signal + 25% CSAT readiness + 25% relationship readiness + 25% adoption/utilization.
